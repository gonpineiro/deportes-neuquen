$("#tabla_solicitudes_aprobadas td").click(function (node) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      $("#tipo_empleo-span-aprobada").html(
        data.tipo_empleo === "1"
          ? "Con manipulación de alimentos"
          : "Sin manipulación de alimentos"
      );
      $("#renovacion-span-aprobada").html(
        data.renovacion === "1" ? "Sí" : "No"
      );
      /* observaciones */
      if (data.observaciones == (null || "")) {
        $("#observaciones-span-aprobada").html("No presenta");
      } else {
        $("#observaciones-span-aprobada").html(
          charsetFormat(data.observaciones)
        );
      }
      ...
      if (verTipoArchivo(data.path_comprobante_pago)) {
        $("#comprobante-pago-span-aprobada").html(
          '<a href="' +
            data.path_comprobante_pago +
      ...
      $("#muni-capa-span-aprobada").html(
        data.municipalidad_nqn === "1" ? "SI" : "NO"
      );
      $("#nombre-capa-span-aprobada").html(
        data.nombre_capacitador
          ? charsetFormat(data.nombre_capacitador) +
              " " +
              charsetFormat(data.apellido_capacitador)
          : ""
      );
      $("#matricula-span-aprobada").html(data.matricula);
      $("#lugar-capa-span-aprobada").html(
        charsetFormat(data.lugar_capacitacion)
      );
      $("#fecha-capa-span-aprobada").html(formatDate(data.fecha_capacitacion));
      if (verTipoArchivo(data.path_certificado)) {
        $("#certificado-capa-aprobada").html(
          '<a href="' +
            data.path_certificado +
            '" target="_blank"><img style="width:100%" src="' +
            data.path_certificado +
            '"></a>'
        );
...
$("#tabla_nuevas_solicitudes td").click(function (node) {
  const id = node.currentTarget.parentNode.id;
  window.location.href = "../administrar/info_solicitud.php?id=" + id;
});
...
function imprimirLibreta() {
  var idReferencia = document.getElementById(
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      var tipoEmpleo =
        data.tipo_empleo === "1"
          ? "CON Manipulación Alimentos"
          : "SIN Manipulación Alimentos";
